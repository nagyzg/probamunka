...
SugarWidgetListView.prototype.display = function() {
	...
	html += '<th width="20%" nowrap="nowrap">'
			+ GLOBAL_REGISTRY['meeting_strings']['LBL_NAME'] + '</th>';
	html += '<th width="20%" nowrap="nowrap">'
			+ GLOBAL_REGISTRY['meeting_strings']['LBL_EMAIL'] + '</th>';
	html += '<th width="20%" nowrap="nowrap">'
			+ GLOBAL_REGISTRY['meeting_strings']['LBL_PHONE'] + '</th>';
	...
	for (var i = 0; i < GLOBAL_REGISTRY['result_list'].length; i++) {
		...
		html += '<td><img src="' + GLOBAL_REGISTRY.config['site_url']
				+ '/index.php?entryPoint=getImage&themeName='
				+ SUGAR.themes.theme_name + '&imageName=' + bean.module
				+ 's.gif"/></td>';
		...
		html += '<input type="button" id="invitees_add_'
				+ (i + 1)
...
SugarWidgetSchedulerSearch.submit = function(form) {
	...
	if (form.search_last_name.value != '') {
		conditions[conditions.length] = {
			"name" : "last_name",
			"op" : "starts_with",
			"value" : form.search_last_name.value
		}
	}
	...
	global_request_registry[req_id] = [
			GLOBAL_REGISTRY['widget_element_map'][form.id], 'refresh_list' ];
}
...
SugarWidgetSchedulerSearch.prototype.display = function() {
	...
	form.setAttribute('onsubmit',
			'SugarWidgetSchedulerSearch.submit(this);return false;');
	...
	input3.setAttribute('value',
			GLOBAL_REGISTRY['meeting_strings']['LBL_SEARCH_BUTTON']);
	...
	button1.setAttribute('onclick',
			'SugarWidgetSchedulerSearch.showCreateForm(\'Contacts\');');
	...
	button2.setAttribute('onclick',
			'SugarWidgetSchedulerSearch.showCreateForm(\'Leads\');');
	...
	form1.setAttribute('onsubmit',
			'SugarWidgetSchedulerSearch.createInvitee(this); return false;');
	...
	button3.setAttribute('onclick',
			'SugarWidgetSchedulerSearch.createInvitee(this.form);');
	...
	addToValidate('createInviteeForm', 'last_name', 'last_name', true,
...
SugarWidgetSchedulerSearch.showCreateForm = function(module) {
	...
	document.getElementById('empty-search-message').style.display = 'none';
	if (typeof document.createInviteeForm.first_name != 'undefined'
			&& typeof document.schedulerwidget.search_first_name != 'undefined')
		document.createInviteeForm.first_name.value = document.schedulerwidget.search_first_name.value;
	if (typeof document.createInviteeForm.last_name != 'undefined'
			&& typeof document.schedulerwidget.search_last_name != 'undefined')
		document.createInviteeForm.last_name.value = document.schedulerwidget.search_last_name.value;
	if (typeof document.createInviteeForm.email1 != 'undefined'
			&& typeof document.schedulerwidget.search_email != 'undefined')
...
SugarWidgetSchedulerSearch.resetSearchForm = function() {
	if (GLOBAL_REGISTRY.scheduler_search_obj
			&& document.forms[GLOBAL_REGISTRY.scheduler_search_obj.form_id]) {
...
SugarWidgetSchedulerSearch.createInvitee = function(form) {
	...
	document.getElementById('create-invitee-btn').setAttribute('disabled',
			'disabled');
	document.getElementById('cancel-create-invitee-btn').setAttribute(
			'disabled', 'disabled');
	...
	var callback = {
		success : function(response) {
			...
			document.getElementById('create-invitee-btn').removeAttribute(
					'disabled');
			document.getElementById('cancel-create-invitee-btn')
...
SugarWidgetScheduler.update_time = function() {
	...
	if (has_meridiem) {
		GLOBAL_REGISTRY.focus.fields.time_start = hour_start + time_separator
				+ minute_start + meridiem;
	} else {
		GLOBAL_REGISTRY.focus.fields.time_start = hour_start + time_separator
				+ minute_start;
	}
	// on Task form not exsist
	// added kz 2016.02.10.
	if (typeof document.forms[form_name].duration_hours !== 'undefined') {
		GLOBAL_REGISTRY.focus.fields.duration_hours = document.forms[form_name].duration_hours.value;
	}
	// on Task form not exsist
	// added kz 2016.02.10.
	if (typeof document.forms[form_name].duration_minutes !== 'undefined') {
		GLOBAL_REGISTRY.focus.fields.duration_minutes = document.forms[form_name].duration_minutes.value;
	}
	GLOBAL_REGISTRY.focus.fields.datetime_start = SugarDateTime
			.mysql2jsDateTime(GLOBAL_REGISTRY.focus.fields.date_start,
					GLOBAL_REGISTRY.focus.fields.time_start);
...
SugarWidgetSchedulerAttendees.prototype.init = function() {
	...
	var curdate = new Date(dtstart.getFullYear(), dtstart.getMonth(), dtstart
			.getDate(), dtstart.getHours() - this.start_hours_before, 0);
	if (typeof (GLOBAL_REGISTRY.focus.fields.duration_minutes) == 'undefined') {
		GLOBAL_REGISTRY.focus.fields.duration_minutes = 0;
	}
	GLOBAL_REGISTRY.focus.fields.datetime_end = new Date(dtstart.getFullYear(),
			dtstart.getMonth(), dtstart.getDate(), dtstart.getHours()
					+ parseInt(GLOBAL_REGISTRY.focus.fields.duration_hours),
			dtstart.getMinutes()
					+ parseInt(GLOBAL_REGISTRY.focus.fields.duration_minutes),
			0);
	var has_start = false;
	var has_end = false;
	for (i = 0; i < this.hours * this.segments; i++) {
		...
		if (has_start == false
				&& GLOBAL_REGISTRY.focus.fields.datetime_start.getTime() <= curdate
						.getTime()) {
			obj.is_start = true;
			has_start = true;
		}
		if (has_end == false
				&& GLOBAL_REGISTRY.focus.fields.datetime_end.getTime() <= curdate
						.getTime()) {
			obj.is_end = true;
			has_end = true;
		}
		this.timeslots.push(obj);
		curdate = new Date(curdate.getFullYear(), curdate.getMonth(), curdate
				.getDate(), curdate.getHours(), curdate.getMinutes()
				+ minute_interval);
	}
	SugarWidgetSchedulerSearch.resetSearchForm();
}
...
SugarWidgetSchedulerAttendees.prototype.display = function() {
	...
	var html = '<h3>'
			...
			+ '</h3><table id ="schedulerTable">';
	html += '<tr class="schedulerTopRow">';
	html += '<th colspan="' + ((this.hours * this.segments) + 2) + '"><h4>'
			+ top_date + '</h4></th>';
	html += '</tr>';
	...
	for (var i = 0; i < (this.timeslots.length / this.segments); i++) {
		...
		html += '<th scope="col" colspan="' + this.segments + '">' + form_hours
				+ am_pm + '</th>';
	}
	html += '<td>&nbsp;</td>';
	html += '</tr>';
	...
	if (this.parentNode.childNodes.length < 1)
		this.parentNode.innerHTML += '<div class="schedulerDiv">' + html
				+ '</div>';
	else
...
SugarWidgetSchedulerAttendees.form_add_attendee = function(list_row) {
	if (typeof (GLOBAL_REGISTRY.result_list[list_row]) != 'undefined'
			&& typeof (GLOBAL_REGISTRY.focus.users_arr_hash[GLOBAL_REGISTRY.result_list[list_row].fields.id]) == 'undefined') {
...
SugarWidgetScheduleRow.prototype.load = function(thetableid) {
	...
	if (typeof (GLOBAL_REGISTRY['freebusy_adjusted']) == 'undefined'
			|| typeof (GLOBAL_REGISTRY['freebusy_adjusted'][this.focus_bean.fields.id]) == 'undefined') {
		global_request_registry[req_count] = [ this, 'display' ];
...
SugarWidgetScheduleRow.prototype.display = function() {
	SUGAR.util
			.doWhen(
					"document.getElementById('" + this.thetableid
							+ "') != null",
					function() {
						var tr;
						this.thetable = document
								.getElementById(this.thetableid);
						if (typeof (this.element) != 'undefined') {
							...
							tr = this.thetable
									.insertRow(this.thetable.rows.length);
						}
						...
						var img = '<img align="absmiddle" src="index.php?entryPoint=getImage&themeName='
								...
								+ '&imageName='
								+ this.focus_bean.module + 's.gif"/>&nbsp;';
						td.innerHTML = img;
						td.innerHTML = td.innerHTML;
						if (this.focus_bean.fields.full_name)
							td.innerHTML += ' '
									+ this.focus_bean.fields.full_name;
						else
