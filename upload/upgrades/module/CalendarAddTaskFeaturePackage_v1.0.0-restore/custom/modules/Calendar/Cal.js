...
CAL.arrange_column = function(column) {
	...
	function find_overlapping(el, start, end, level, ol_group) {
		...
		for (var i = start; i < end; i++) {
			if (i >= slot_count)
				break;
			if (typeof slots[i].childNodes != 'undefined'
					&& typeof slots[i].childNodes[0] != 'undefined') {
				var pos = 0;
				if (i == start) {
					var node = slots[i].childNodes[0];
					while (node.nextSibling
							&& CAL.contains(affected_items, node.id)) {
						node = node.nextSibling;
						pos++;
					}
				}
				var current = slots[i].childNodes[pos];
				var slots_takes = parseInt(current
						.getAttribute('duration_coef'));
				...
				if (slots_takes > 0) {
					var k = find_overlapping(current, i, i + slots_takes,
							level + 1, ol_group);
					if (k > depth)
						depth = k;
				}
...
CAL.create_item = function(params) {
	...
	if (typeof item.repeat_parent_id != "undefined"
			&& item.repeat_parent_id != "")
	...
	if (params.type == "basic") {
		...
		el.style.top = parseInt(params.position * CAL.slot_height
				- params.slot.childNodes.length * (CAL.slot_height + 1))
				+ "px";
	} else {
		el.style.height = parseInt((CAL.slot_height + 1) * params.duration_coef
				- 1)
				+ "px";
...
CAL.make_draggable = function(id, type) {
	...
	dd.onDragOver = function(e, id) {
		...
		if (type == "advanced")
			this.el.childNodes[0].childNodes[1].childNodes[0].innerHTML = slot
					.getAttribute('time');
	}
...
CAL.make_resizable = function(id, slot) {
	...
	resize
			.on(
					...
					function(e) {
						elm_id = id;
						var duration = e.height / (CAL.slot_height + 1)
								* CAL.t_step;
						...
						var new_size = (duration / CAL.t_step)
								* (CAL.slot_height + 1) - 1;
						...
						if (el) {
							...
							var callback = {
								success : function(o) {
									...
									if (res.access == 'yes') {
										CAL.update_vcal();
										CAL.clear_additional_details(el
												.getAttribute("record"));
							...
							ajaxStatus.showStatus(SUGAR.language.get(
									'app_strings', 'LBL_SAVING'));
		...
		for (w = 0; w < portions_count; w++) {
			...
			CAL.each(portion, function(i, v) {
				...
				if (slot) {
					...
					if (item.ts_start < start)
						time_start = "...&nbsp;";
					var head_text = CAL.get_header_text(item.type, time_start,
					...
					YAHOO.util.Event.on(el, "click", function() {
						if (this.getAttribute('detail') == "1")
							CAL.load_form(this.getAttribute('module_name'),
									this.getAttribute('record'), false);
					});
					slot.appendChild(el);
			...
			h = parseInt((max_pos + 1) * CAL.slot_height
					+ (CAL.slot_height + 1));
...
CAL.add_item_to_grid = function(item) {
	...
	var head_text = CAL.get_header_text(item.type, item.time_start, item.name,
			item.record);
	...
	var item_text = SUGAR.language.languages.app_list_strings[item.type
			+ '_status_dom'][item.status];
	...
	YAHOO.util.Event.on(el, "click", function() {
		if (this.getAttribute('detail') == "1")
			CAL.load_form(this.getAttribute('module_name'), this
					.getAttribute('record'), false);
	});
	//debugger;
	var slot;
	if (slot = CAL.get("t_" + time_cell + suffix)) {	
		...
		if (CAL.view == "shared") {
			var end_time = $("#" + slot.id).parents("div:first").children(
					"div:last").attr("time");
			var end_time_id = $("#" + slot.id).parents("div:first").children(
					"div:last").attr("id");
			if (end_time && end_time_id) {
				...
				if (share_coef < duration_coef)
					el.style.height = parseInt((CAL.slot_height + 1)
							* share_coef - 1)
							+ "px";
			}
		}
...
CAL.get_header_text = function(type, time_start, text, record) {
	var start_text = (CAL.view == 'month') ? ("<span class='start_time'>"
			+ time_start + "</span> " + text) : text;
	return start_text;
}
CAL.cut_record = function(id) {
	...
	el.style.height = parseInt((CAL.slot_height + 1) * duration_coef - 1)
			+ "px";
}
...
CAL.open_edit_dialog = function(params) {
	...
	if (typeof params != "undefined"
			&& typeof params.stay_on_tab != "undefined" && params.stay_on_tab)
		stay_on_tab = true;
	if (!stay_on_tab) {
...
CAL.reset_edit_dialog = function() {
	...
	// kz 2016.02.10.
	CAL.get("radio_task").removeAttribute("disabled");
...
CAL.fill_repeat_data = function() {
	if (CAL.enable_repeat
			&& (CAL.get("current_module").value == "Meetings" || CAL
					.get("current_module").value == "Calls")) {
...
CAL.fill_repeat_tab = function(data) {
	...
	if (typeof data.default_repeat_until != "undefined"
		...
		var callback = {
			success : function(o) {
				...
				if (res.access == 'yes') {
					...
					CAL.script_evaled = false;
					fc.innerHTML = '<script type="text/javascript">CAL.script_evaled = true;</script>'
							+ res.html;
					...
					CAL.get("radio_meeting").setAttribute("disabled",
							"disabled");
					// kz 2016.02.10.
					CAL.get("radio_task").setAttribute("disabled", "disabled");
					eval(res.gr);
					SugarWidgetScheduler.update_time();
					if (CAL.record_editable) {
						CAL.enable_buttons();
...
CAL.add_item = function(item) {
	...
	if (typeof item.edit_all_recurrences != "undefined"
			&& item.edit_all_recurrences == 'true')
		edit_all_recurrences = true;
	if (CAL.view != 'shared') {
		var arr = new Array();
		if (CAL.enable_repeat && edit_all_recurrences) {
			var nodes = CAL.query("div.act_item[repeat_parent_id='"
					+ item.record + "']");
			CAL.each(nodes, function(i, v) {
				var record = nodes[i].getAttribute("record");
				if (!CAL.contains(arr, record))
...
CAL.move_activity = function(box_id, slot_id, ex_slot_id) {
	...
	if (u = CAL.get(box_id)) {
		if (s = CAL.get(slot_id)) {
			...
			var start_text = CAL.get_header_text(CAL.act_types[u
					.getAttribute('module_name')], s.getAttribute('time'),
					' ... ', u.getAttribute('record'));
...
CAL.disable_buttons = function() {
	...
	if (CAL.enable_repeat) {
		CAL.get("btn-edit-all-recurrences")
				.setAttribute("disabled", "disabled");
		CAL.get("btn-remove-all-recurrences").setAttribute("disabled",
				"disabled");
	}
}
...
CAL.dialog_create = function(cell) {
	...
	if (CAL.view == 'shared') {
		...
		if (parentWithUserValues.length > 1) {
			...
			while (theUser) {
				if (theUser.getAttribute("user_name")
						&& theUser.getAttribute("user_id")) {
...
CAL.goto_date_call = function() {
	...
	window.location.href = "index.php?module=Calendar&view=" + CAL.view
			+ "&day=" + date_arr[1] + "&month=" + date_arr[0] + "&year="
			+ date_arr[2];
}
	...
	var callback = {
		success : function(result) {
			...
			GLOBAL_REGISTRY.freebusy[v] = SugarVCalClient.parseResults(
					result.responseText, false);
			GLOBAL_REGISTRY.freebusy_adjusted[v] = SugarVCalClient
					.parseResults(result.responseText, true);
			SugarWidgetScheduler.update_time();
		}
	};
